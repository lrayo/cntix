...
export default function CrearLead({setOnChange}) {
  ...
  useEffect(() => {
    function check() {
      if(formData && formData.nombre_lead && formData.apellido_lead && formData.nombre_municipio && formData.telefono) 
        setCheckForm(false)
    ...
    const handleChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await fetch('/api/suncom', {
            method: 'POST',
    ...
    <div className="flex justify-center">
        <button type="button" onClick={()=>document.getElementById('modalCrearLead').showModal()} 
                className="btn btn-primary text-white">Crear Lead</button>
