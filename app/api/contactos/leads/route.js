...
export async function GET(req) {
    ...
    const id = searchParams.get("id"); // Obtiene el parámetro "id"
      ...
      try {
        ...
        const result = await pool.request().query(`SELECT * FROM empresas_leads_contactos WHERE id_empresalead = ${id} AND estado_empleads_contacto = 1`);
...
export async function POST(req) {
      try {
        ...
        const query = `INSERT INTO empresas_leads_contactos (
          id_empresalead, id_tipo_doc, numero_doc_contacto,
          ...
          id_rol, estado_empleads_contacto, observaciones,
        ...
        const result = await pool.request()
          ...
          .input('estado_empleads_contacto', sql.VarChar, "1")
    ...
    export async function PUT(req) {
      try {
        ...
        const pool = await connectDB();
        const query = `UPDATE empresas_leads_contactos 
                       SET id_tipo_doc = @id_tipo_doc, numero_doc_contacto = @numero_doc_contacto,
                       ...
                       WHERE id_empleadscontac = @id_empleadscontac`;
        
        const result = await pool.request()
                       .input('id_empleadscontac', sql.Int, body.id_empleadscontac)
                       .input('id_tipo_doc', sql.Int, body.id_tipo_doc)
                       .input('numero_doc_contacto', sql.VarChar, body.numero_doc_contacto)
                       .input('nombre_contacto', sql.VarChar, body.nombre_contacto)
                       .input('apellido_1_contacto', sql.VarChar, body.apellido_1_contacto)
...
export async function DELETE(req) {
  try {
      ...
      const pool = await connectDB();
      const query = "UPDATE empresas_leads_contactos SET estado_empleads_contacto = '0', sys_usrmod = @sys_usrmod WHERE id_empleadscontac = @id_empleadscontac";

      const result = await pool.request()
                   .input('id_empleadscontac', sql.Int, id)
                   .input('sys_usrmod', sql.VarChar, session.user.email)
                   .query(query);
